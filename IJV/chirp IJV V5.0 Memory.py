...
CMD_HELLO = 0x0514
CMD_HELLO_RESP = 0x0515
CMD_READ_PHYSICAL = 0x0564
CMD_READ_PHYSICAL_RESP = 0x0565
CMD_WRITE_PHYSICAL = 0x0566
CMD_WRITE_PHYSICAL_RESP = 0x0567

FW5_SPI_USER_BASE = 0x000000
FW5_SPI_BANK0_BASE = 0x020000
FW5_SPI_BANK_SIZE = 0x4000
FW5_RUNTIME_CONFIG_OFFSET = 0x1550
FW5_RUNTIME_CONFIG_MAGIC = 0x35474643
FW5_ACTIVE_BANK_OFFSET = 19
FW5_BANK_NAME_LOGICAL = 0x5F00
...
def _send_command(serport, data: bytes):
    """Invia un comando app-mode FW5 non offuscato."""
    ...
    command = struct.pack(">H", 0xabcd) + struct.pack("<H", len(payload)) + \
              payload + struct.pack("<H", crc) + struct.pack(">H", 0xdcba)
...
def _receive_reply(serport):
    ...
    reply_size = header[2] | (header[3] << 8)
    cmd = serport.read(reply_size)
    if len(cmd) != reply_size:
    ...
    LOG.debug("Received reply len=0x%4.4x:\n%s" % (len(cmd), util.hexprint(cmd)))
    return cmd
...
#--------------------------------------------------------------------------------
def _is_fw5_firmware(firmware_version):
    text = (firmware_version or "").strip().upper()
    return "FW5" in text

#--------------------------------------------------------------------------------
def _logical_to_physical(offset, bank):
    if offset < START_MEM:
        return FW5_SPI_USER_BASE + offset
    if START_MEM <= offset < END_MEM:
        return FW5_SPI_BANK0_BASE + (bank * FW5_SPI_BANK_SIZE) + (offset - START_MEM)
    raise errors.RadioError("Logical address outside FW5 memory window: 0x%04X" % offset)

#--------------------------------------------------------------------------------
def _read_physical(serport, address, length):
    if length <= 0 or length > 128:
        raise errors.RadioError("Invalid FW5 read length: %d" % length)
    payload = struct.pack("<HHIBB", CMD_READ_PHYSICAL, 6, address, length, 0)
    o = _request_radio(serport, payload, expected_cmd=CMD_READ_PHYSICAL_RESP)
    if len(o) < 10:
        raise errors.RadioError("Short FW5 physical read reply")
    reply_address = struct.unpack_from("<I", o, 4)[0]
    reply_size = o[8]
    status = o[9]
    if reply_address != address or reply_size != length or status != 0:
        raise errors.RadioError(
            "FW5 physical read failed addr=0x%06X len=%d status=%d" %
            (reply_address, reply_size, status))
    return o[10:10 + reply_size]

#--------------------------------------------------------------------------------
def _write_physical(serport, address, data):
    if not data or len(data) > 128:
        raise errors.RadioError("Invalid FW5 write length: %d" % len(data))
    payload = struct.pack("<HHIBB", CMD_WRITE_PHYSICAL, 6 + len(data), address, len(data), 0) + data
    o = _request_radio(serport, payload, expected_cmd=CMD_WRITE_PHYSICAL_RESP)
    if len(o) < 10:
        raise errors.RadioError("Short FW5 physical write reply")
    reply_address = struct.unpack_from("<I", o, 4)[0]
    reply_size = o[8]
    status = o[9]
    if reply_address != address or reply_size != len(data) or status != 0:
        raise errors.RadioError(
            "FW5 refused memory write at 0x%06X. Firmware must allow CHIRP bank writes first." %
            address)
    return True

#--------------------------------------------------------------------------------
def _read_fw5_active_bank(serport):
    try:
        cfg = _read_physical(serport, FW5_RUNTIME_CONFIG_OFFSET, 32)
        if len(cfg) >= (FW5_ACTIVE_BANK_OFFSET + 1):
            magic = struct.unpack_from("<I", cfg, 0)[0]
            bank = cfg[FW5_ACTIVE_BANK_OFFSET]
            if magic == FW5_RUNTIME_CONFIG_MAGIC and 0 <= bank <= 15:
                return bank
    except Exception as e:
        LOG.warning("Unable to read FW5 active bank from CFG5: %s", e)
    return 0
...
def _sayhello(serport):
    hellopacket = struct.pack("<HH", CMD_HELLO, 4) + SESSION_TIMESTAMP
...
def _readmem(serport, offset, length):
    ...
    bank = getattr(serport, "_ijv_fw5_active_bank", 0)
    physical = _logical_to_physical(offset, bank)
    o = _read_physical(serport, physical, length)
    if DEBUG_SHOW_MEMORY_ACTIONS:
        LOG.debug("readmem Received data len=0x%4.4x:\n%s" % (len(o), util.hexprint(o)))
    if len(o) != length:
        raise errors.RadioError("Short FW5 memory read")
    return o
...
def _writemem(serport, data, offset):
    ...
    bank = getattr(serport, "_ijv_fw5_active_bank", 0)
    physical = _logical_to_physical(offset, bank)
    return _write_physical(serport, physical, bytes(data))
...
def do_download(radio):
    ...
    if not _is_fw5_firmware(radio.FIRMWARE_VERSION):
        raise errors.RadioError("This module is for IJV FW5 radios only")

    radio.ACTIVE_BANK = _read_fw5_active_bank(serport)
    serport._ijv_fw5_active_bank = radio.ACTIVE_BANK
    LOG.info("FW5 active bank: %s", radio.ACTIVE_BANK)
    ...
    if len(image) > 0x000B:
        image[0x000B] = radio.ACTIVE_BANK
    bank_name_src = FW5_BANK_NAME_LOGICAL
    bank_name_dst = BANK_NAME_START + (radio.ACTIVE_BANK * BANK_NAME_SIZE)
    if bank_name_dst + BANK_NAME_SIZE <= len(image) and bank_name_src + BANK_NAME_SIZE <= len(image):
        name = bytes(image[bank_name_src:bank_name_src + BANK_NAME_SIZE]).strip(b"\x00\xff ")
        if name:
            image[bank_name_dst:bank_name_dst + BANK_NAME_SIZE] = image[bank_name_src:bank_name_src + BANK_NAME_SIZE]
...
def do_upload(radio):
    ...
    if image_bank is None:
        image_bank = _read_fw5_active_bank(serport)
    if image_bank is None or image_bank < 0 or image_bank > 15:
        image_bank = 0
    radio.ACTIVE_BANK = image_bank
    serport._ijv_fw5_active_bank = image_bank
    LOG.info("FW5 upload bank: %s", image_bank)
...
class UVK5Radio(chirp_common.CloneModeRadio):
    "IJV FW5.0 (memory-only, 500 ch)"
    VENDOR   = "Quansheng"
    MODEL    = "IJV FW5"
    VARIANT  = "V5.0 - Memory Only"
    BAUD_RATE = 38400
